# dictee v1.1.0 Configuration Wizard — Implementation Plan

> **For agentic workers:** REQUIRED: Use superpowers:subagent-driven-development (if subagents available) or superpowers:executing-plans to implement this plan. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Add a step-by-step configuration assistant (wizard) to `dictee-setup.py` with 5 pages, visual radio buttons, real-time microphone level test, and dictation test.

**Architecture:** QStackedWidget with 5 pages in DicteeSetupDialog. Conditional construction: widgets are created once in `__init__()` and placed directly in the correct container (stack or scroll) based on `self.wizard_mode`. No dynamic re-parenting.
...
### Task 1 : --wizard argument in shell script

**Files:**
- Modify: `dictee:83-85` (block `--setup)`)
- Modify: `dictee:10` (usage comment)

- [ ] **Step 1: Add --wizard to dictee script**
...
And update the usage comment line 10 :
```bash
...
```bash
dictee-setup --wizard  # should open without error
dictee --setup --wizard  # same via shell script
```
...
After reading config and before building sections, add conditional branch :
...
Extract existing UI construction code into `_build_classic_ui()`. Create `_build_wizard_ui()` that creates QStackedWidget with 5 empty pages + navigation bar.
...
def _validate_wizard_page(self, idx):
    """Validates current page. Returns True if OK."""
    if idx == 0:  # ASR page: verify model is installed
...
```bash
dictee-setup --wizard  # should display 5 empty pages with functional navigation
```
...
- [ ] **Step 1: Add button to existing bar**
...
def _launch_wizard(self):
    """Closes dialog and reopens in wizard mode."""
    self.reject()
...
```bash
dictee-setup  # classic mode, verify "Assistant" button at bottom left
# Click → should close and reopen in wizard mode
```
...
- [ ] **Step 6: Connect page in _build_wizard_ui()**

Call `self._build_wizard_page_asr()` after pages are created.
...
```bash
git add dictee-setup.py
git commit -m "feat: page 1 wizard — ASR backend with visual radio buttons"
```
...
def _build_wizard_page_shortcuts(self):
    ...
    if self.de_type == "unsupported":
        # tiling WM: display manual commands
...
```bash
git add dictee-setup.py
git commit -m "feat: page 2 wizard — keyboard shortcuts with conflict detection"
```
...
def _build_wizard_page_translation(self):
    ...
    # Visual radio buttons — local first
...
```bash
git add dictee-setup.py
git commit -m "feat: page 3 wizard — translation with local backends first"
```
...
def _build_wizard_page_visual(self):
    ...
    # Use a QScrollArea because this page is dense
    scroll = QScrollArea()
...
Start the thread when arriving on page 4, stop when exiting :
...
Modify `_wizard_next()` and `_wizard_prev()` to call `_start_audio_level()` on entering page 4 and `_stop_audio_level()` on exiting.
...
```bash
git add dictee-setup.py
git commit -m "feat: page 4 wizard — microphone, visual feedback, startup services"
```

---

## Chunk 4 : Page 5, integration, i18n, finishing touches
...
def _run_wizard_checks(self):
    """Runs automatic checks for page 5."""
    checks = {
...
def _check_shortcut_registered(self):
    ...
    return True  # GNOME/unsupported: assume OK
...
def _on_test_dictee(self):
    """Launches transcribe-client subprocess for test."""
...
```bash
git add dictee-setup.py
git commit -m "feat: page 5 wizard — automatic checks and dictation test"
```

---

### Task 10 : Connect _on_apply() to wizard
...
Wizard mode uses `self._current_asr` and `self._current_trans_backend` (strings) instead of ComboBox. Modify `_on_apply()` to read the correct sources :
...
dictee-setup  # should open in wizard mode
# Navigate all pages → Finish → verify dictee.conf created
...
- [ ] **Step 1: Add Microphone section in classic form**

After existing "Visual interface" section, add a "Microphone" QGroupBox with :
- Audio source ComboBox
- Volume slider
- Level bar (AudioLevelThread)

Reuse the same logic as page 4 of wizard.

- [ ] **Step 2: Start AudioLevelThread on opening in classic mode**
...
```bash
dictee-setup  # classic mode, verify microphone section
```
...
Open each `.po` file and translate marked `fuzzy` or empty strings. Main strings:
...
```bash
git add po/
git commit -m "feat: i18n wizard — 6 languages (fr, de, es, it, uk, pt)"
```
...
Follow MEMORY.md checklist: Cargo.toml, build-deb.sh (×3), control, metadata.json, READMEs.
...
```bash
mv ~/.config/dictee.conf ~/.config/dictee.conf.bak
dictee-setup  # should open in wizard mode automatically
# Navigate all pages, verify each section
# Finish → verify dictee.conf created with all keys
mv ~/.config/dictee.conf.bak ~/.config/dictee.conf
```

- [ ] **Step 2: Test wizard forced**

```bash
dictee --setup --wizard  # should open in wizard even with existing config
...
# Verify: "Assistant" button present, microphone section present
# Click "Assistant" → should restart in wizard
```
...
- [ ] **Step 5: Final commit if corrections**

```bash
git add -u
git commit -m "fix: wizard integration corrections"
```
